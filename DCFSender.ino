...
namespace timer_1 {
 ...
 // Timer 1 : Port B pin 2 
        // enable OC1B pin for output (digital pin PB2 that is pin16 on pro micro)
        DDRB |= 1<<6;
        ...
        // TOM. TOP is defined as 0xFF when WGM2:0 = 3, and OCR0A when MGM2:0 = 7. In non-
        ...
        TCCR1A = 1<< WGM10 | 1<<WGM11 | // Fast PWM
                 1<<COM1B1;             // Clear OC2B on Compare Match
 
        TCCR1B = 1<<CS10 |  // no Prescaler
                 1<<WGM12 | 1<<WGM13 ;  // Fast PWM
 
        OCR1A = ticks_per_period - 1;  // period length
 
        OCR1B = pwm_full_carrier;  // duty cycle
 
        TIMSK1 = 1<<OCIE1A;  // enable match interrupts
    }
}
...
ISR(TIMER1_COMPA_vect) {
    ...
    accumulated_fractional_ticks += timer_1::nominator;
    if (accumulated_fractional_ticks < timer_1::denominator) {
        OCR1A = timer_1::ticks_per_period - 1;
        RXLED0;
    } else {
        OCR1A = timer_1::ticks_per_period;
        accumulated_fractional_ticks -= timer_1::denominator;
        RXLED1;
    }
}
...
namespace timer_3 {
    void setup() {
        TIMSK3 = 0;  // disable timer1 interrupts
 
        TCCR3A = 0;  // do not toggle or change timer IO pins
 
        // generate interrupts at 10 Hz
        OCR3A = 6250 - 1;  // (16000000 / 256 / 10) - 1
 
        TCCR3B = (1<<WGM32) |  // CTC using OCR1A
                 (1<<CS32);    // set prescaler to 256
 
        TIMSK3 = (1 << OCIE3A);  // enable OCR3A match interrupt
    }
}
...
void modulate() {
    ...
    if (times_100ms == 0) {
        ...
        if (times_100ms != stop_modulation_after_times_100ms) {
            OCR1B = timer_1::pwm_modulated_carrier;
        }
    }
 
    if (times_100ms == stop_modulation_after_times_100ms) {
        OCR1B = timer_1::pwm_full_carrier;
    }
...
ISR(TIMER3_COMPA_vect) {
    sei();
    // ensure that timer 1 can interrupt timer 3
    modulate();
}
